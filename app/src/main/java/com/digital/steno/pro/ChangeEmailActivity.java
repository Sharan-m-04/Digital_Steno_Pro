		...
		button1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (edittext1.getText().toString().equals("") && edittext2.getText().toString().equals("")) {
					...
					if (FirebaseAuth.getInstance().getCurrentUser().getEmail().equals(edittext1.getText().toString())) {
						newmail = edittext2.getText().toString();
						auth.getCurrentUser().updateEmail(newmail) .addOnCompleteListener(new OnCompleteListener<Void>() { @Override public void onComplete(Task<Void> task) { if (task.isSuccessful()) { showMessage ("User email address updated."); } 
								else{
						...
						SketchwareUtil.showMessage(getApplicationContext(), "Please Enter Current E-Mail Address");
					}
				}
			}
		});
